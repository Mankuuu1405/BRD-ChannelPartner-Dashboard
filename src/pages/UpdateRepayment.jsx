...
const reasonOptions  = ["Advance Clawback", "Overpayment", "Policy Breach Fine", "Dispute Recovery", "Other"];
...
function Toast({ msg, type, onClose }) {
  ...
  return <div className={`urr-toast urr-toast-${type}`}><span>{type === "success" ? "✓" : "✕"}</span>{msg}</div>;
}

function AddModal({ onClose, onAdd }) {
  const [form, setForm] = useState({ agentId:"", agentName:"", type:"DSA", totalOwed:"", recovered:"0", emiAmt:"", emiDay:"1", reason:"Advance Clawback", status:"Active", startDate:"", dueDate:"" });
  ...
  const handleAdd = () => {
    ...
    const newId = `RR${String(Date.now()).slice(-3)}`;
    onAdd({ ...form, id: newId, totalOwed: parseInt(form.totalOwed), recovered: parseInt(form.recovered || 0), emiAmt: parseInt(form.emiAmt) });
  };
    ...
    <div className="urr-modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="urr-modal">
        ...
        <div className="urr-modal-footer">
          <button className="urr-btn-ghost" onClick={onClose}>Cancel</button>
          <button className="urr-btn-primary" onClick={handleAdd}>Add Record</button>
        </div>
      </div>
    </div>
  );
...
function EditDrawer({ record, onClose, onSave }) {
    ...
    <div className="urr-drawer-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="urr-drawer">
        ...
        <div className="urr-drawer-body">
          ...
          <div className="urr-edit-grid">
            ...
            <div className="urr-ef">
              <label>Deduction Day</label>
              <select value={form.emiDay} onChange={e => f("emiDay", e.target.value)}>
                <option value="1">1st</option><option value="5">5th</option>
                <option value="10">10th</option><option value="15">15th</option>
              </select>
            </div>
        ...
        <div className="urr-drawer-footer">
          <button className="urr-btn-ghost" onClick={onClose}>Cancel</button>
          <button className="urr-btn-primary" onClick={() => onSave(form)}>Save Changes</button>
        </div>
      </div>
    </div>
  );
...
export default function UpdateRepayment() {
  ...
  const filtered = useMemo(() => records.filter(r => {
    const matchS = r.agentName.toLowerCase().includes(search.toLowerCase()) || r.id.includes(search) || r.agentId.includes(search);
  ...
  const handleAdd = (rec) => {
    setRecords(p => [rec, ...p]);
    setShowAdd(false);
    showToast(`Recovery record added for ${rec.agentName}`);
  };

  const handleSave = (updated) => {
    setRecords(p => p.map(r => r.id === updated.id ? updated : r));
    setEditRecord(null);
    showToast(`Record ${updated.id} updated successfully`);
  };
  ...
  const totalOwed      = records.reduce((s, r) => s + r.totalOwed, 0);
  const totalRecovered = records.reduce((s, r) => s + r.recovered, 0);
  ...
  return (
    <div className="urr-root">
      ...
      {editRecord && <EditDrawer record={editRecord} onClose={() => setEditRecord(null)} onSave={handleSave} />}
      ...
      <div className="urr-header">
        ...
        <div className="urr-header-actions">
          ...
          <button className="urr-btn-primary" onClick={() => setShowAdd(true)}>
      ...
      {/* ── 4 STAT CARDS (matching UpdatePayout layout) ── */}
      <div className="urr-summary-band">

        {/* Card 1 — Total Owed (indigo) */}
        ...
        {/* Card 4 — Overall Recovery % (purple) */}
      ...
      <div className="urr-main-card">
        ...
        <div className="urr-toolbar">
          <div className="urr-search-wrap">
            ...
            <input placeholder="Search by agent or record ID..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
              ...
              <div key={r.id} className="urr-record-card">
                <div className="urr-card-top">
                  <div className="urr-card-agent">
                    ...
                    <div>
                      ...
                      <div className="urr-card-meta">{r.agentId} · <span className="urr-type-badge" style={{ background: typeColors[r.type]?.bg, color: typeColors[r.type]?.color }}>{r.type}</span></div>
                    </div>
                  </div>
                ...
                <div className="urr-card-emi">
                  <div className="urr-emi-item">
                    ...
                    <span>₹{r.emiAmt.toLocaleString()} / {r.emiDay === "1" ? "1st" : r.emiDay === "5" ? "5th" : r.emiDay === "10" ? "10th" : "15th"}</span>
                  </div>
                  ...
                  <div className="urr-emi-item">
                    ...
                    <span>Due: {new Date(r.dueDate).toLocaleDateString("en-IN", { day:"numeric", month:"short", year:"numeric" })}</span>
                  </div>
                </div>

                <div className="urr-card-actions">
                  <button className="urr-card-edit-btn" onClick={() => setEditRecord(r)}>
                  ...
                  <button className="urr-card-del-btn" onClick={() => handleDelete(r.id)}>
