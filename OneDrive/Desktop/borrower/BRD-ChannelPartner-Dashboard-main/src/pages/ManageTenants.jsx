...
const initialTenants = [
  { id: "TN001", name: "Mumbai Metro Finance", city: "Mumbai",    state: "Maharashtra", region: "West",  agents: 42, active: true,  plan: "Enterprise", monthlyVolume: "₹2.4Cr", contactName: "Vikram Shah",    contactEmail: "vikram@mmf.com",    contactPhone: "9876543210", joined: "Mar 2024", modules: ["Agent Mgmt","Payout","Performance","Offers"] },
  { id: "TN002", name: "Delhi Capital Brokers", city: "Delhi",    state: "Delhi",       region: "North", agents: 28, active: true,  plan: "Pro",        monthlyVolume: "₹1.1Cr", contactName: "Pooja Arora",    contactEmail: "pooja@dcb.com",     contactPhone: "9123456789", joined: "Jun 2024", modules: ["Agent Mgmt","Payout","Performance"] },
  { id: "TN003", name: "Bangalore Tech Loans",  city: "Bangalore",state: "Karnataka",   region: "South", agents: 55, active: true,  plan: "Enterprise", monthlyVolume: "₹3.1Cr", contactName: "Suresh Menon",   contactEmail: "suresh@btl.com",    contactPhone: "9988776655", joined: "Jan 2024", modules: ["Agent Mgmt","Payout","Performance","Offers","KYC"] },
  { id: "TN004", name: "Hyderabad Housing Co",  city: "Hyderabad",state: "Telangana",   region: "South", agents: 19, active: false, plan: "Starter",    monthlyVolume: "₹42L",   contactName: "Nandini Reddy",  contactEmail: "nandini@hhc.com",   contactPhone: "9012345678", joined: "Sep 2024", modules: ["Agent Mgmt","Payout"] },
  { id: "TN005", name: "Chennai Fintech Hub",   city: "Chennai",  state: "Tamil Nadu",  region: "South", agents: 31, active: true,  plan: "Pro",        monthlyVolume: "₹88L",   contactName: "Arjun Pillai",   contactEmail: "arjun@cfh.com",     contactPhone: "8877665544", joined: "Nov 2024", modules: ["Agent Mgmt","Payout","Performance"] },
  { id: "TN006", name: "Pune Wealth Partners",  city: "Pune",     state: "Maharashtra", region: "West",  agents: 22, active: true,  plan: "Pro",        monthlyVolume: "₹76L",   contactName: "Deepa Joshi",    contactEmail: "deepa@pwp.com",     contactPhone: "8765432109", joined: "Feb 2025", modules: ["Agent Mgmt","Payout"] },
];
...
const emptyTenant = { id:"", name:"", city:"", state:"", region:"West", agents:0, active:true, plan:"Starter", monthlyVolume:"", contactName:"", contactEmail:"", contactPhone:"", joined:"", modules:["Agent Mgmt"] };

function Toast({ msg, type, onClose }) {
  React.useEffect(() => { const t = setTimeout(onClose, 3000); return () => clearTimeout(t); }, []);
  return <div className={`mt-toast mt-toast-${type}`}><span>{type==="success"?"✓":"✕"}</span>{msg}</div>;
}
...
function TenantModal({ tenant, onClose, onSave }) {
  ...
  const toggleModule = (m) => setForm(p => ({ ...p, modules: p.modules.includes(m) ? p.modules.filter(x=>x!==m) : [...p.modules, m] }));
  ...
  const handleSave = () => {
    ...
    onSave({ ...form, id: tenant?.id || `TN${String(Date.now()).slice(-3)}`, joined: tenant?.joined || new Date().toLocaleDateString("en-IN",{month:"short",year:"numeric"}) });
  };
      ...
      <div className="mt-modal">
        <div className="mt-modal-header">
          <div className="mt-modal-title-wrap">
            <div className="mt-modal-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div>
            <div>
              <h2>{isEdit ? "Edit Tenant" : "Add New Tenant"}</h2>
              <p>{isEdit ? `Editing ${tenant.id}` : "Onboard a new tenant organisation"}</p>
            </div>
          </div>
          <button className="mt-close" onClick={onClose}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
          ...
          <div className="mt-modules-grid">
            {allModules.map(m=>(
              <button key={m} type="button" className={`mt-module-btn ${form.modules.includes(m)?"selected":""}`} onClick={()=>toggleModule(m)}>
                ...
                {form.modules.includes(m) && <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="12" height="12" style={{marginLeft:"auto"}}><polyline points="20 6 9 17 4 12"/></svg>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-modal-footer">
          ...
          <button className="mt-btn-primary" onClick={handleSave}>{isEdit?"Save Changes":"Add Tenant"}</button>
        </div>
      </div>
    </div>
  );
...
export default function ManageTenants() {
  ...
  const filtered = useMemo(() => tenants.filter(t => {
    const ms = t.name.toLowerCase().includes(search.toLowerCase()) || t.id.includes(search) || t.city.toLowerCase().includes(search.toLowerCase());
    const mr = filterReg==="All" || t.region===filterReg;
    const mp = filterPlan==="All" || t.plan===filterPlan;
    return ms && mr && mp;
  }), [tenants, search, filterReg, filterPlan]);

  const handleSave = (tnt) => {
    if (tenants.find(t=>t.id===tnt.id)) { setTenants(p=>p.map(t=>t.id===tnt.id?tnt:t)); showToast(`"${tnt.name}" updated`); }
    else { setTenants(p=>[tnt,...p]); showToast(`"${tnt.name}" added successfully`); }
    setModal(null);
  };
  ...
  const toggleActive = (id) => {
    ...
    showToast(`${t?.name} ${t?.active?"deactivated":"activated"}`, t?.active?"error":"success");
  };
  ...
  return (
    <div className="mt-root">
      ...
      {modal && <TenantModal tenant={modal==="add"?null:modal} onClose={()=>setModal(null)} onSave={handleSave} />}

      {/* Header */}
      <div className="mt-header">
        <div className="mt-header-left">
          <div className="mt-header-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="22" height="22"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div>
          <div>
      ...
      <div className={`mt-layout ${viewTenant?"split":""}`}>
        ...
        <div className="mt-list-panel">
          ...
          <div className="mt-toolbar">
            ...
            <div className="mt-filter-row">
              <select className="mt-select" value={filterReg} onChange={e=>setFilterReg(e.target.value)}>
                {["All","North","South","East","West","Central"].map(r=><option key={r} value={r}>{r==="All"?"All Regions":r}</option>)}
              </select>
              <select className="mt-select" value={filterPlan} onChange={e=>setFilterPlan(e.target.value)}>
                {["All","Starter","Pro","Enterprise"].map(p=><option key={p} value={p}>{p==="All"?"All Plans":p}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="mt-tenant-list">
            {filtered.map(t=>(
              <div key={t.id} className={`mt-tenant-row ${viewTenant?.id===t.id?"active":""} ${!t.active?"inactive":""}`} onClick={()=>setViewTenant(t)}>
                ...
                <div className="mt-row-right">
                  <span className="mt-plan-badge" style={{background:planColors[t.plan]?.bg,color:planColors[t.plan]?.color,border:`1px solid ${planColors[t.plan]?.border}`}}>{t.plan}</span>
                  <span className={`mt-dot ${t.active?"on":"off"}`} />
                </div>
              </div>
            ))}
            {filtered.length===0 && <div className="mt-empty-list">No tenants found</div>}
          </div>
          ...
          <div className="mt-detail-panel">
            <div className="mt-detail-header">
              ...
              <div className="mt-detail-badges">
                <span className="mt-plan-badge" style={{background:planColors[viewTenant.plan]?.bg,color:planColors[viewTenant.plan]?.color,border:`1px solid ${planColors[viewTenant.plan]?.border}`}}>{viewTenant.plan}</span>
                <span className={`mt-status-pill ${viewTenant.active?"active":"inactive"}`}>{viewTenant.active?"Active":"Inactive"}</span>
              </div>
            </div>
            ...
            <div className="mt-detail-body">
              ...
              <div className="mt-detail-actions">
                ...
                <button className="mt-action-del" onClick={()=>handleDelete(viewTenant.id)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg>
                </button>
              </div>
            </div>
          </div>
