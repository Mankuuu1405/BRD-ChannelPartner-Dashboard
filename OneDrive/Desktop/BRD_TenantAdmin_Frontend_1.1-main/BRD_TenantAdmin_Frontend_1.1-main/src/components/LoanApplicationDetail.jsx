...
export default function LoanApplicationDetail({ app, onClose }) {
  ...
  const handleStatusChange = async (newStatus) => {
    setProcessing(true);
    try {
      await loanApplicationAPI.updateStatus(
      ...
      alert(
        "Status Update Failed: " + (err.response?.data?.detail || err.message)
      );
    } finally {
      setProcessing(false);
    }
  ...
  const handleVideoAction = async (decision) => {
    try {
      await loanApplicationAPI.verifyVideo(
        app.id || app.application_id,
        decision,
        "Manual Review"
      );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 transition-opacity backdrop-blur-sm">
      <div className="bg-gray-50 w-full max-w-5xl h-[95vh] shadow-2xl flex flex-col animate-slide-in rounded-xl overflow-hidden">
        {/* ---------------- HEADER ---------------- */}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Field
                ...
                value={app.aadhaar_number || "xxxx-xxxx-xxxx"}
                badge="API Verified"
                badgeColor="blue"
              />
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-100">
                <div>
                  ...
                  <div className="text-sm text-gray-800 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">
                    {app.res_address_line1}
                    <br />
                    {app.res_address_line2 && (
                      <>
                        {app.res_address_line2}
                        <br />
                      </>
                    )}
                    {app.res_city}, {app.res_state} -{" "}
                    <strong>{app.res_pincode}</strong>
                    <br />
                ...
                <div>
                  <h4 className="text-xs font-bold text-gray-500 uppercase mb-3 flex items-center gap-2">
                    <BuildingOfficeIcon className="h-4 w-4" /> Business
                    Address (Geo-Fence)
                  </h4>
                  <div className="text-sm text-gray-800 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">
                    {app.office_address_line1 || "N/A"}
                    <br />
                    {app.office_address_line2 && (
                      <>
                        {app.office_address_line2}
                        <br />
                      </>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Field
                    ...
                    value={`₹${Number(app.monthly_income).toLocaleString()}`}
                  />
                  <Field
          ...
          {/* SECTION E: DOCUMENT UPLOADS - MODIFIED FOR BUSINESS LOAN */}
          <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <SectionHeader
              title="E. Document Uploads"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                />
                <DocItem
                  name="Business Address Proof (Rental Agreement/Ownership Deed)"
                  status="Verified"
                  type="PDF"
                />
                <DocItem
                ...
                />
                <DocItem
                  name="Financial Documents (ITR for 3 Years + GST Returns)"
                  status="Pending Review"
                  type="PDF"
                />
                <DocItem
                  name="Business Proof (Shop & Establishment Certificate/Trade License)"
                  status="Pending Review"
                  type="PDF"
                />
              </div>
              ...
              <div className="mt-6 border-t pt-4">
                <h4 className="text-xs font-bold text-orange-600 uppercase mb-3 flex items-center gap-2">
                  <VideoCameraIcon className="h-4 w-4" /> Mandatory:
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-center">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-orange-700">
                      Required for unsecured business loans.
                    </p>
                  </div>
...
const Field = ({ label, value, badge, badgeColor, highlight }) => (
  <div
    className={`flex flex-col ${
      highlight ? "bg-yellow-50 p-2 -m-2 rounded" : ""
    }`}
  >
