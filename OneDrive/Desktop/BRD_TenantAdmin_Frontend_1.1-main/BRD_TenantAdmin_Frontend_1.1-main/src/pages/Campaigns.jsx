    ...
    <div className="p-10 max-w-[1600px] mx-auto min-h-screen bg-slate-50">
      ...
      <div className="flex justify-between items-end mb-10">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-primary-600 text-white px-6 py-3 rounded-xl font-bold uppercase tracking-widest hover:bg-primary-700 transition shadow-lg shadow-primary-600/20"
        >
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 text-slate-400 font-extrabold uppercase tracking-widest text-[10px]">
              <tr>
                ...
                <th className="px-8 py-5">Engaged</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {MOCK_CAMPAIGNS.filter(c => activeTab === 'ALL' || c.type === activeTab).map((campaign) => (
                <tr key={campaign.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-wide
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${campaign.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                      <span className="text-xs font-bold text-slate-600">{campaign.status}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 font-mono text-slate-600 font-bold">{campaign.sent.toLocaleString()}</td>
                  <td className="px-8 py-6 font-mono text-slate-600 font-bold">{campaign.clicked.toLocaleString()}</td>
                  <td className="px-8 py-6">
                    <div className="font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg inline-block">
                      {campaign.conversion}
                    </div>
                  </td>
      ...
      {/* CREATE MODAL (Basic Mockup) */}
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-8 animate-fade-in">
            <h3 className="text-2xl font-black text-slate-900 mb-6">Start New Campaign</h3>
