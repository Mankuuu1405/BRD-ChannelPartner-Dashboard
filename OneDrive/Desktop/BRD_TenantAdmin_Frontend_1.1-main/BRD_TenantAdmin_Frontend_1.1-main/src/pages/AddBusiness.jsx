    ...
    <div className="relative group" ref={wrapperRef}>
      ...
      >
        {selectedIds.length === 0 ? (
          <span className="text-slate-300 pl-2 select-none font-medium">
            {placeholder}
          </span>
        ) : (
          selectedIds.map((id) => (
            <span
              key={id}
              className="bg-primary-50 text-primary-700 px-2.5 py-1 rounded-lg text-xs font-bold border border-primary-100 flex items-center gap-1"
            >
              {getLabel(id)}
              <span
        ...
        <div className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 overflow-y-auto animate-fade-in p-1">
          {options.length > 0 ? (
            options.map((option) => (
              ...
              >
                <div
                  className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedIds.includes(option.id) ? "bg-primary-600 border-primary-600" : "border-slate-300 bg-white"}`}
                >
                ...
                <span
                  className={`text-sm font-bold ${selectedIds.includes(option.id) ? "text-primary-700" : "text-slate-600"}`}
                >
                  {option.name}
                </span>
              </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium">
            Register a new business entity under your tenant account.
          </p>
        </div>
      </div>
        ...
        <div className="bg-slate-50/50 border-b border-slate-100 px-10 py-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium">
              Please provide official registration details.
            </p>
          </div>
        </div>
        ...
        <div className="p-10 space-y-10">
          <div className="space-y-6">
            <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
              <span className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center font-bold text-sm shadow-md">
                1
              </span>
