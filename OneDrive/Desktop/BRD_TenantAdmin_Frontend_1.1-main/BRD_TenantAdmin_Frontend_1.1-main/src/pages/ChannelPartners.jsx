...
export default function ChannelPartners() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [partnerData, stateData] = await Promise.all([
          partnerAPI.getAll(),
  ...
  useEffect(() => {
    locationAPI.getCountries().then(data => setCountries(data));
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setValidationError("Failed to save partner to database.");
    }
  };
  ...
  return (
    <div className="p-12 max-w-[1800px] mx-auto min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Channel Partners</h1>
          <p className="text-slate-500 text-lg font-medium">Onboard and manage external sales partners (DSA / Agents).</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/80 text-slate-500 font-extrabold uppercase tracking-widest text-[11px] border-b border-slate-200">
              <tr>
                ...
                <th className="px-8 py-6">Company & Type</th>
                ...
                <tr key={p.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    ...
                    <div className="flex gap-2 mt-1">
                      <span className="inline-block text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">
                        {p.partnerType}
                      </span>
                      <span className="inline-block text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">
                        {p.role_type}
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-600"><span className="text-slate-400"><Icons.Mail /></span> {p.email}</div>
                      <div className="flex items-center gap-2 text-xs font-medium text-slate-500"><span className="text-slate-400"><Icons.Phone /></span> {p.mobileNumber}</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-2 items-start">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border ${p.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                        {p.status}
                      </span>
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditing(p); setValidationError(""); }}
