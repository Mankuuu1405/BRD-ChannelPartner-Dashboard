...
updatePartner,
deletePartner,
} from "../services/thirdPartyUsersService";
...
export default function ThirdPartyUsers() {
  ...
  // State Keys renamed to match Model expectations via toSnakeCase
  // addressLine1 -> address_line1, panNumber -> pan_number, isActive -> is_active
  const initialForm = {
    firstName: '',
    lastName: '',
    email: '',
    ...
    confirmPassword: '',
    isVerified: false,
    isActive: true, // Boolean to match Model is_active
  };
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [countryData] = await Promise.all([locationAPI.getCountries()]);
        const userData = await getPartners();
  ...
  useEffect(() => {
    const country = editing ? editing.country : form.country;
    if (country) {
      locationAPI.getStates(country).then(data => {
        setStates(data);
        if (!editing) {
  ...
  useEffect(() => {
    const state = editing ? editing.state : form.state;
    if (state) {
      locationAPI.getCities(state).then(data => {
        setCities(data);
        if (!editing) {
  ...
  const handleSave = async () => {
    ...
    try {
      // We send the camelCase state directly. 
      // service.js toSnakeCase will transform it to Django snake_case fields.
      const payload = { ...form };
      
      // Remove UI-only fields
      delete payload.confirmPassword;

      const newUser = await createPartner(payload);
      ...
      const message = err.data ? Object.values(err.data).flat().join(" ") : err.message || "Failed to create third party user.";
  ...
  const handleUpdate = async () => {
    try {
      // Send camelCase state. Service handles snake_case conversion.
      // We do NOT send password in update (usually handled by separate endpoint or ignored)
      const { password, confirmPassword, ...updatePayload } = editing;
      
      const updated = await updatePartner(editing.id, updatePayload);
      setItems(items.map(u => u.id === editing.id ? updated : u));
      setEditing(null);
      setValidationError("");
    } catch (e) {
      console.error(e);
      const message = e.data ? Object.values(e.data).flat().join(" ") : "Update failed.";
      setValidationError(message);
    }
  };
  ...
  return (
    <div className="p-12 max-w-[1800px] mx-auto min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Third Party Users</h1>
          <p className="text-slate-500 text-lg font-medium">Manage external partners like Valuers and Advisors.</p>
        </div>
        <Button onClick={() => { setCreating(true); setForm(initialForm); }} icon={<Icons.Plus />}>Add Third Party</Button>
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/80 text-slate-500 font-extrabold uppercase tracking-widest text-[11px] border-b border-slate-200">
              <tr>
                ...
                <th className="px-8 py-6">Organization & Role</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((u) => (
                <tr key={u.id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="font-bold text-slate-800 text-base">{u.firstName} {u.lastName}</div>
                    <div className="text-xs font-bold text-blue-600 mt-1 flex items-center gap-1">
                      <Icons.User /> User ID: #{u.id?.toString().slice(0,8)}
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="font-bold text-slate-700">{u.organization_name}</div>
                    <span className="inline-block mt-1 text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">
                      {u.thirdPartyType}
                    </span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-600"><span className="text-slate-400"><Icons.Mail /></span> {u.email}</div>
                      <div className="flex items-center gap-2 text-xs font-medium text-slate-500"><span className="text-slate-400"><Icons.Phone /></span> {u.mobileNumber}</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-2 items-start">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border ${u.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                        {u.isActive ? 'Active' : 'Inactive'}
                      </span>
                      {u.isVerified && <div className="flex items-center gap-1 text-[10px] font-bold text-blue-600"><Icons.ShieldCheck /> Verified</div>}
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => { setEditing(u); setValidationError(""); }}
              ...
              {!items.length && <tr><td colSpan={6} className="p-12 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">No users found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-slate-50 px-8 py-6 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center"><Icons.User /></div>
                <div>
                  <h2 className="text-xl font-extrabold text-slate-900">Add Third Party User</h2>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Create New Partner Account</p>
                </div>
              </div>
            ...
            <div className="p-8 overflow-y-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-2"><h4 className="text-xs font-bold text-blue-900 uppercase tracking-widest border-b border-blue-100 pb-2 mb-2">Identity & Role</h4></div>
                ...
                <FormGroup label="Organization Name" required>
                  <InputBox value={form.organizationName} onChange={e => setForm({ ...form, organizationName: e.target.value })} placeholder="Company Name" icon={<Icons.Building />} />
                </FormGroup>
                ...
                <FormGroup label="Third Party Type" required>
                  ...
                  ]} value={form.thirdPartyType} onChange={e => setForm({ ...form, thirdPartyType: e.target.value })} placeholder="Select Type" icon={<Icons.Briefcase />} />
                </FormGroup>

                <FormGroup label="PAN Number">
                  <InputBox value={form.panNumber} onChange={e => setForm({ ...form, panNumber: e.target.value })} placeholder="PAN Card Number" icon={<Icons.Identification />} />
                </FormGroup>
              </div>

              {/* 2. Contact */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-2"><h4 className="text-xs font-bold text-blue-900 uppercase tracking-widest border-b border-blue-100 pb-2 mb-2">Contact Details</h4></div>
                <FormGroup label="Mobile No" required>
                  <InputBox value={form.mobileNumber} onChange={e => setForm({ ...form, mobileNumber: e.target.value })} placeholder="+91 98765 43210" icon={<Icons.Phone />} />
                </FormGroup>
                <FormGroup label="Email" required>
                  <InputBox type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="user@example.com" icon={<Icons.Mail />} />
                </FormGroup>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormGroup label="Address Line 1"><InputBox value={form.addressLine1} onChange={e => setForm({ ...form, addressLine1: e.target.value })} icon={<Icons.MapPin />} /></FormGroup>
                <FormGroup label="Address Line 2"><InputBox value={form.addressLine2} onChange={e => setForm({ ...form, addressLine2: e.target.value })} icon={<Icons.MapPin />} /></FormGroup>

                <FormGroup label="Country">
                  <SelectBox options={countries} value={form.country} onChange={e => setForm({ ...form, country: e.target.value })} />
                </FormGroup>
              ...
              {/* 4. Security */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-2"><h4 className="text-xs font-bold text-blue-900 uppercase tracking-widest border-b border-blue-100 pb-2 mb-2">Security & Access</h4></div>
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormGroup label="Organization Name" required>
                  <InputBox value={editing.organizationName} onChange={e => setEditing({ ...editing, organizationName: e.target.value })} icon={<Icons.Building />} />
                </FormGroup>
                ...
                <FormGroup label="Third Party Type" required>
                  <SelectBox options={[
                    ...
                    { value: 'ADVISOR', label: 'Advisor' },
                    { value: 'CONSULTANT', label: 'Consultant' }
                  ]} value={editing.thirdPartyType} onChange={e => setEditing({ ...editing, thirdPartyType: e.target.value })} icon={<Icons.Briefcase />} />
                </FormGroup>

                <FormGroup label="PAN Number">
                  <InputBox value={editing.panNumber} onChange={e => setEditing({ ...editing, panNumber: e.target.value })} icon={<Icons.Identification />} />
                </FormGroup>
              </div>
              ...
              {/* 3. Address */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormGroup label="Address Line 1"><InputBox value={editing.addressLine1} onChange={e => setEditing({ ...editing, addressLine1: e.target.value })} icon={<Icons.MapPin />} /></FormGroup>
                <FormGroup label="Address Line 2"><InputBox value={editing.addressLine2} onChange={e => setEditing({ ...editing, addressLine2: e.target.value })} icon={<Icons.MapPin />} /></FormGroup>
