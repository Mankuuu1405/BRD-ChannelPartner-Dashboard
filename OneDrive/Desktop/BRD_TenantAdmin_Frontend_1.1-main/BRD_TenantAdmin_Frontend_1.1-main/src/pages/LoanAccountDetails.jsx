      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white mb-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-400 font-medium text-lg">Borrower: <span className="text-white">{account.borrower_name}</span></p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-8 mt-12 relative z-10">
          <div>
            ...
            <p className="text-3xl font-mono font-bold">₹{Number(account.amount).toLocaleString('en-IN')}</p>
          </div>
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Outstanding</p>
            <p className="text-3xl font-mono font-bold text-orange-400">₹{Number(account.outstanding).toLocaleString('en-IN')}</p>
          </div>
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Next EMI</p>
            <p className="text-3xl font-mono font-bold">{account.next_emi_date || 'N/A'}</p>
          ...
          <div>
            ...
            <div className="flex items-end gap-2">
              <span className="text-3xl font-mono font-bold">{account.tenure_completed || 0}</span>
              <span className="text-slate-400 mb-1 font-bold">/ {account.tenure_total || 0} Months</span>
            </div>
            ...
            <tbody className="text-sm divide-y divide-slate-100">
              {schedule.map((row) => (
                <tr key={row.inst} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-4 font-bold">₹{Number(row.amount).toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4 text-slate-500">₹{Number(row.principal).toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4 text-slate-500">₹{Number(row.interest).toLocaleString('en-IN')}</td>
