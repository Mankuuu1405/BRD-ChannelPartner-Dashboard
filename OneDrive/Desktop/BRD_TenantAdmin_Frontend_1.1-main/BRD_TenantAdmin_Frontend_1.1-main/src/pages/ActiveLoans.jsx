...
import axiosInstance from "../utils/axiosInstance"; // Using your centralized axios
import { 
  MagnifyingGlassIcon, 
  FunnelIcon, 
...
export default function ActiveLoans() {
  ...
  const fetchActiveLoans = async () => {
    setLoading(true);
    try {
      // API call to fetch only active/disbursed loans
  ...
  return (
    <div className="p-8 min-h-screen bg-slate-50 font-sans">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Active Loan Portfolio</h1>
          <p className="text-slate-500 font-medium">Manage disbursements, repayment tracking, and closures.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={fetchActiveLoans} className="px-4 py-2 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50">
            Refresh Data
          </button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-100 text-slate-500 font-extrabold uppercase text-[11px] tracking-widest border-b border-slate-200">
              <tr>
                <th className="px-8 py-5">Loan Account</th>
                <th className="px-8 py-5">Product Type</th>
                <th className="px-8 py-5">Disbursed Amount</th>
                <th className="px-8 py-5">Outstanding</th>
                <th className="px-8 py-5">Next EMI</th>
              ...
              {loading ? (
                ...
                filteredLoans.map((loan) => (
                  <tr key={loan.id} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">{loan.product_name}</span>
                    </td>
                    ...
                    <td className="px-8 py-5">
                       <div className="text-sm font-bold text-slate-800">{loan.next_emi_date}</div>
                    ...
                    <td className="px-8 py-5 text-right">
                       <button 
                         ...
                         className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition"
                       >
